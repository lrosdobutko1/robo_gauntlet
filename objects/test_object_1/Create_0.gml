...
nearby_radius = sprite_get_width(spr_obstacle)/2 + 10;
show_debug_message(nearby_radius);
...
function chase_the_player(_closest_ally, _speed, _target_x, _target_y) {
	
	show_debug_message(_closest_ally);
	
	if (instance_exists(_closest_ally)) {
		
		if (point_distance(x, y, _closest_ally.x, _closest_ally.y) <= 30) _speed *= 0.01;
		else _speed = walk_speed;
	}
	...
	vx = lengthdir_x(_speed, point_direction(x, y, px, py));
	vy = lengthdir_y(_speed, point_direction(x, y, px, py));
